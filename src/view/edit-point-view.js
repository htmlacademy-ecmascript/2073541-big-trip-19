...
const getOffersTemplate = (offers, pointOffers) => pointOffers.map((offer) =>
  `<div class="event__offer-selector">
        <input class="event__offer-checkbox  visually-hidden"
          id="event-offer-${he.encode(String(offer.id))}" type="checkbox"
          name=${he.encode(offer.title)} ${offers.includes(offer.id) ? 'checked' : ''} value="${he.encode(String(offer.id))}">
...
const getTypeTemplate = (type, allOffers) => allOffers.map((offer) =>
  `<div class="event__type-item">
    <input id="event-type-${he.encode(offer.type)}-${he.encode(String(offer.id))}"
      class="event__type-input  visually-hidden" type="radio"
      name="event-type" value="${he.encode(offer.type)}" ${type === offer.type ? 'checked' : ''}>
    <label class="event__type-label  event__type-label--${he.encode(offer.type)}"
      for="event-type-${he.encode(offer.type)}-${he.encode(String(offer.id))}">${capitalizeFirstLetter(offer.type)}</label>
  </div>`).join('');
  ...
  if(destination && destination.description) {
    ...
    <h3 class="event__section-title  event__section-title--destination">Destination</h3>
     ...
     <div class="event__photos-container">
        ...
        `<img class="event__photo" src="${he.encode(src)}" alt="${he.encode(pictureDescription)}">`).join('')}
        </div>
      </div>
   </section>`);
  }
...
const getDestinationInput = (destination, id) => {
  if(destination) {
    return `<input class="event__input  event__input--destination" id="event-destination-${he.encode(String(id))}" type="text" name="event-destination"
  value=${he.encode(destination.name)} list="destination-list-${he.encode(String(id))}" >`;
  }
  return `<input class="event__input  event__input--destination" id="event-destination-${he.encode(String(id))}" type="text" name="event-destination"
 value list="destination-list-${he.encode(String(id))}" >`;
};
        ...
        <header class="event__header">
          ...
          <div class="event__field-group  event__field-group--time">
            ...
            <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time"
              value='${he.encode(dayjs(dateFrom).format('DD/MM/YY HH:mm'))}'>
            &mdash;
            <label class="visually-hidden" for="event-end-time-1">To</label>
            <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time"
              value='${he.encode(dayjs(dateTo).format('DD/MM/YY HH:mm'))}'>
          </div>
